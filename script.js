...
//-----------------------Search Function for Current City----------------------------------//
function searchCity(cityname) {

    var queryURL = "https://api.openweathermap.org/data/2.5/weather?q=" + cityname + "&units=imperial&appid=717e377cd5a9f932dd2379a623e32cf6";
    var queryURLforcast = "https://api.openweathermap.org/data/2.5/forecast?q=" + cityname + "&units=imperial&appid=717e377cd5a9f932dd2379a623e32cf6";
    ...
    $.ajax({
       ...
       var mainDate = moment().format('L');
        ...
        //create HTML for city information, the date, an icon representation of weather conditions, the temperature, the humidity, the wind speed, and the UV index......
        ...
        var uvEl = $("<p>").text("UV Index: " + response.uv.index);
...
//--------------------------------------------5 Day frocast call ---------------------------------------//
    ...
    $.ajax({
        ...
        for (var i = 0; i < results.length; i += 8) {
            ...
            var uv = results[i].main.uv;
   
            //create tags with the result items information. city name, the date, an icon representation of weather conditions, the temperature, the humidity, the wind speed, and the UV index.
            var date = $("<h5 class='card-title'>").text(setD);
            var Temp = $("<p class='card-text'>").text("Temp: " + temp);;
            var Hum = $("<p class='card-text'>").text("Humidity " + hum);;
            var windSpeed = $("<p class='card-text'>").text("Wind Speed " + hum);;
            var UV = $("<p class='card-text'>").text("UV Index " + hum);;
